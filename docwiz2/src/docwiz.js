...
export default class TransitionsExample extends React.PureComponent {
  ...
  async componentDidMount() {
    ...
    this.setState({ items: ["DocWiz"] });
    this.t1 = setTimeout(() => this.setState({ items: ["DocWiz"] }), 1500);
    this.t2 = setTimeout(() => this.setState({ items: ["DocWiz"] }), 3000);
    this.t3 = setTimeout(() => this.setState({ items: ["Welcome"] }), 4500);
  }
